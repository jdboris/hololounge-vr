...
export function Localizationprovider({ children }) {
  /**
   * @type {[string, (language: LANGUAGE) => void]}
   */
  ...
  function localize(text, newLanguage = language) {
    ...
    const english = text;
    const localizedText = localizeRaw(text, newLanguage);
    cache[localizedText] = cache[localizedText] || english;
 ...
 * @property {(newLanguage: LANGUAGE) => void} setLanguage
 * @property {(text: string, language: LANGUAGE) => string} localize
 */
 ...
 */
function localizeRaw(english, newLanguage) {
  if (newLanguage == "ja-JP") {
    switch (english) {
      case `Games`:
        return `ゲーム`;
      case `Catalog`:
